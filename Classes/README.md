...
- Value threshold: 1, 2
```
The first metric checker ensures that the `AchievementService` class has low complexity (the WMC metric is often calculated as the sum of the cyclomatic complexities of individual methods). When it detects an issue, the challenge gives the following hint to the learner: _The AchievementService is a stereotypical service class. The complexity that these classes encapsulate concerns the coordination of multiple objects, not the complexity of the logic found in the class's methods. Ensure you have delegated any business or technical logic to the appropriate classes._

The second metric checker checks if the class interacts with a sufficient number of other classes (our ideal solution includes the `Achievement`, the `AchievementCollection`, and the `AchievementFileRepository`). When it detects an issue, the challenge gives the following hint to the learner: _The main responsibility of service objects is coordinating multiple objects to produce a result for the client. Such classes often have high efferent coupling since their code invokes other classes. Ensure your service is interacting with multiple classes that have clear responsibilities._

The final metric checker examines if the class has one or two methods, where we expect the other methods to be moved to other classes. When it detects an issue, the challenge gives the following hint to the learner: _When properly designed, a service should have one simple method for each "service" it provides to the client. In the case of this challenge, the main service provided by the AchievementService is the AwardAchievement function._
